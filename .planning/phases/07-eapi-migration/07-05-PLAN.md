...
3. In `TestRealMetadataExtraction`, the tests call `python_bridge.get_book_metadata_complete()` which already uses EAPI internally. The assertions about `terms`, `booklists`, `description`, `ipfs_cids`, `rating`, `download` fields need review:
   - EAPI `get_book_info` returns different field structure than HTML scraping
   - Check what `get_book_metadata_complete` actually returns now (it delegates to `get_enhanced_metadata` which uses EAPI)
   - Update assertions to match actual EAPI response fields. If fields like `terms`, `booklists`, `ipfs_cids` are NOT available via EAPI (they were HTML-scraped), mark those test methods with `pytest.mark.skip(reason="Field not available via EAPI")` rather than deleting them (preserves intent for future)

4. In `TestRealAdvancedSearch`, verify `python_bridge.search_advanced()` still exists and works with EAPI. If it was removed/changed during migration, update or skip those tests.
...
Run: `cd /home/rookslog/workspace/projects/zlibrary-mcp && grep 'get_enhanced_metadata' __tests__/python/integration/test_real_zlibrary.py` — should show the import
  ...
  <done>Integration test file has zero references to deprecated HTML functions or BeautifulSoup. All imports use EAPI equivalents. File parses without syntax errors.</done>
</task>
...
<success_criteria>
Integration tests reference only EAPI functions. No deprecated HTML imports remain. TestHTMLStructureValidation removed. File is syntactically valid.
</success_criteria>
