...
must_haves:
  truths:
    ...
    - "All 12 tools are registered via McpServer.registerTool()"
...
**Tool count:** There are exactly 12 tools (12 ParamsSchema definitions: search_books, full_text_search, get_download_history, get_download_limits, get_recent_books, download_book_to_file, process_document_for_rag, get_book_metadata, search_by_term, search_by_author, fetch_booklist, search_advanced). Verify all 12 are registered by counting `registerTool(` calls.
  </action>
  <verify>
- `npm run build` succeeds (exit code 0, no TypeScript errors)
- `grep -c "registerTool" dist/index.js` shows 12 (the exact tool count)
...
4. `grep -c "registerTool" src/index.ts` → 12
