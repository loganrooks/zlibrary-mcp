...
class TestGetEnhancedMetadata:
    ...
    async def test_get_enhanced_metadata_with_client(self):
        ...
        mock_client.get_book_info = AsyncMock(return_value={
            'book': {'title': 'Test', 'description': 'A test book'}
        })

        metadata = await get_enhanced_metadata(
            book_id=123, book_hash='abc',
...
class TestLegacyCompatibility:
    ...
    def test_empty_metadata_structure(self):
        ...
        expected_keys = [
            'description', 'terms', 'booklists', 'rating',
            'ipfs_cids', 'series', 'categories',
            'isbn_10', 'isbn_13', 'quality_score',
        ]
