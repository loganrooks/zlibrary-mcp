...
def test_stage_1_strategy_thresholds(garbled_page_region):
    ...
    phil_config = QualityPipelineConfig(strategy="philosophy", garbled_threshold=0.9)
    _phil_result = _stage_1_statistical_detection(garbled_page_region, phil_config)

    # Technical strategy (aggressive)
    tech_config = QualityPipelineConfig(strategy="technical", garbled_threshold=0.6)
    _tech_result = _stage_1_statistical_detection(garbled_page_region, tech_config)
...
def test_stage_2_runs_on_clean_text(mock_detect, clean_page_region, quality_config):
    ...
    mock_detect.return_value = XMarkDetectionResult(
        has_xmarks=False, xmark_count=0, confidence=0.0, flags={"no_crossings_found"}
    )

    # Stage 2 should RUN (not skip) even on clean text
    result, xmark_result = _stage_2_visual_analysis(
        clean_page_region, Path("fake.pdf"), 0, quality_config
    )
...
def test_stage_2_graceful_degradation_no_opencv(garbled_page_region, quality_config):
    ...
    result, xmark_result = _stage_2_visual_analysis(
        garbled_page_region, Path("fake.pdf"), 0, quality_config
    )
...
def test_stage_3_low_confidence_skips_recovery(garbled_page_region, quality_config):
    ...
    garbled_page_region.quality_flags = {"high_symbols"}
    garbled_page_region.quality_score = (
        0.5  # 50% quality = 50% garbled confidence (below 0.8 threshold)
    )
...
def test_stage_3_high_confidence_needs_recovery(garbled_page_region, quality_config):
    ...
    garbled_page_region.quality_score = (
        0.1  # 10% quality = 90% garbled confidence (above 0.8 threshold)
    )
...
@patch("lib.rag_processing.XMARK_AVAILABLE", True)
@patch("lib.strikethrough_detection.detect_strikethrough_enhanced")
def test_pipeline_stops_at_sous_rature(
    mock_detect, garbled_page_region, quality_config
):
...
def test_pipeline_proceeds_to_stage_3(garbled_page_region, quality_config):
    ...
    with (
        ...
        if result.quality_score < 0.2:  # High garbled confidence
            assert (
                "recovery_needed" in result.quality_flags
                or "low_confidence" in result.quality_flags
            )
...
def test_config_from_env_defaults():
    ...
    # Clear environment
    for key in [
        "RAG_ENABLE_QUALITY_PIPELINE",
        "RAG_DETECT_GARBLED",
        "RAG_QUALITY_STRATEGY",
    ]:
    ...
    # Verify defaults
    assert config.enable_pipeline is True  # Default 'true'
    assert config.detect_garbled is True
    assert config.detect_strikethrough is True
...
def test_config_feature_flags():
    ...
    try:
        ...
        assert config.detect_garbled is False
    finally:
...
def test_stage_1_performance(benchmark):
    ...
    region = PageRegion(
        ...
        spans=[
            TextSpan(
                text="This is a sample paragraph with normal text content " * 10,
                formatting=set(),
            )
        ],
        bbox=(50, 100, 500, 150),
