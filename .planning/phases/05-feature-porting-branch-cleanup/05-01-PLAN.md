...
4. **Verify enhanced_metadata.py return structure**: Before implementing the filter, inspect the actual return dict from `lib/enhanced_metadata.py` to confirm:
   - `terms` is returned as `[{name, count}]` (list of objects with name and count keys)
   - `booklists` is returned as `[{id, hash, title, bookCount}]` (list of objects with those keys)
   - `ipfs_cid` is a string field
   - If the structure differs (e.g. terms are flat strings instead of objects), add a mapping step in the TypeScript filter to reshape into the expected nested format before returning.
  </action>
  <verify>
    `npm run build` succeeds. Check that the schema includes the `include` parameter by inspecting the compiled output. Verify the response structure matches expected nested format by tracing enhanced_metadata.py return values.
  </verify>
  <done>
...
6. **End-to-end integration**: Verify get_book_metadata tool works when invoked through MCP by running `echo '{"method":"tools/call","params":{"name":"get_book_metadata","arguments":{"bookId":"test","bookHash":"test"}}}' | node dist/index.js` (or use existing integration test harness if available). Confirm the response includes core fields and respects the `include` parameter filtering.
</verification>
...
- get_book_metadata tool invocable end-to-end (MCP client → Node → Python → response with correct structure)
