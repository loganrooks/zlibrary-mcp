...
def analyze_derrida_page():
    ...
    for idx, block in enumerate(text_blocks):
        ...
        if ")(" in text or "~" in text or ")" in text:
            ...
            for i, char in enumerate(text):
                if char in "()~":
                    ...
                    print(
                        f"   Context around '{char}': ...{text[context_start:context_end]}..."
                    )

        # Run garbled detection on this block
        config = GarbledDetectionConfig(
            symbol_density_threshold=0.25, repetition_threshold=0.7, min_text_length=10
        )
        ...
        print("  Metrics:")
        print(
            f"    Symbol density: {result.metrics.get('symbol_density', 0):.3f} (threshold: 0.25)"
        )
        print(f"    Entropy: {result.metrics.get('entropy', 0):.3f}")
        print(
            f"    Repetition: {result.metrics.get('repetition_ratio', 0):.3f} (threshold: 0.7)"
        )
...
def analyze_word_level_detection():
    ...
    clean_paragraph = (
        "This is a normal philosophical argument about Being and consciousness. " * 5
    )
    xmarked_word = ")("  # Corrupted "is"
    mixed_paragraph = clean_paragraph.replace(
        "is a", f"{xmarked_word} a", 1
    ...
    if not result_mixed.is_garbled:
        ...
        print(
            f"   EXPLANATION: One 2-char X-mark in {total_chars}-char paragraph = {expected_density:.1%} symbols"
        )
        print(
            f"                This is {expected_density / 0.25:.0%} of the 25% threshold"
        )
    ...
    if result_word.is_garbled:
        print("   ✅ FLAGGED: When isolated, X-mark IS detected as garbled")
