...
must_haves:
  truths:
    - "No content duplication between footnotes and margin annotations in output"
    - "All body text pages present in output (zero recall loss)"
    - "Page numbers stripped from body text — not present as standalone lines"
    - "Front matter pages excluded from body text but captured in document_metadata"
    - "TOC structure captured in document_metadata, not mixed into body"
    - "Margins readable inline in body as [margin: ...] annotations"
    - "Footnotes appear in separate output section, not in body text"
  artifacts:
  ...
  key_links:
    ...
    - from: "lib/rag/pipeline/writer.py"
      to: "context['page_number_map']"
      via: "reads page number map to strip page numbers from body"
      pattern: "page_number_map"
    - from: "lib/rag/pipeline/writer.py"
      to: "context['toc_map']"
      via: "reads TOC map to populate metadata and exclude TOC pages from body"
      pattern: "toc_map"
    - from: "lib/rag/pipeline/writer.py"
      to: "context['front_matter']"
      via: "reads front matter info to exclude front matter pages from body and populate metadata"
...
# Verify context flows between detectors (Blocker fix: mock pipeline run)
from unittest.mock import MagicMock
from lib.rag.pipeline.models import DetectorScope
context = {}
# Simulate footnote detector populating context
page_detectors = get_registered_detectors(scope=DetectorScope.PAGE)
fn_detector = next(d for d in page_detectors if d['name'] == 'footnotes')
mg_detector = next(d for d in page_detectors if d['name'] == 'margins')
# Run footnote detector with mock page
mock_page = MagicMock()
mock_page.get_text.return_value = {'blocks': []}
try:
    fn_detector['func'](mock_page, 1, context)
except Exception:
    pass  # detector may fail on mock, but context key should be set
# Verify footnote_bboxes key exists (or would exist with real data)
print(f'Context after footnote detector: {list(context.keys())}')

# Verify document-level detectors populate context
doc_detectors = get_registered_detectors(scope=DetectorScope.DOCUMENT)
doc_context = {}
mock_doc = MagicMock()
for det in doc_detectors:
    try:
        det['func'](mock_doc, doc_context)
    except Exception:
        pass
expected_keys = ['page_number_map', 'toc_map', 'front_matter']
for key in expected_keys:
    # Keys may not populate on mock doc, but the detector should at least attempt
    print(f'  Document context key \"{key}\": {\"present\" if key in doc_context else \"not set (ok with mock)\"}')
print('Context flow verification complete')
"
```
  </verify>
  <done>Pipeline runner orchestrates document-level pre-pass + page-level loop + compositor + output building; context flows verified between detectors</done>
</task>
...
1. `build_document_output(classified_pages: Dict[int, List[BlockClassification]], context: dict, include_metadata: bool = False) -> DocumentOutput`:
   ...
   - **Consumes document-level context values from detector pre-pass:**
     - `context["page_number_map"]` → Used to identify and strip page number text from body blocks. For each page, if a block's text matches the mapped page number string, exclude it from body_text output.
     - `context["toc_map"]` → Stored in `document_metadata["toc"]` as structured data. TOC page ranges (if available) used to exclude TOC pages from body_text.
     - `context["front_matter"]` → Stored in `document_metadata["front_matter"]`. Front matter page numbers used to exclude those pages entirely from body_text output (skip pages in the front_matter page range).
