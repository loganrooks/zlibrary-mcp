...
must_haves:
  truths:
    ...
    - "Output sections clearly labeled: footnotes have page headers, margins have [margin:] markers, metadata has structured keys"
  artifacts:
...
8. `test_output_sections_clearly_labeled()` (BODY-02 verification):
   - Process a scholarly PDF with footnotes and margins
   - Assert footnotes output contains page headers matching pattern `## Page \d+`
   - Assert body_text contains margin annotations matching pattern `\[margin: .+\]`
   - Assert document_metadata has structured keys: at minimum `toc` or `front_matter` key present as dict/list (not raw text)
   - This verifies that each output section is clearly labeled and machine-parseable
...
```
  ...
  <done>All 8 integration tests pass, verifying multi-file output, confidence scores, dedup, stripping, section labeling, and backward compat</done>
</task>
