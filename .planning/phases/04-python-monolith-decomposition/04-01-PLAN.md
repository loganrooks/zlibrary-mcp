...
IMPORTANT: Do not change any function signatures. Copy verbatim including all BUG-X FIX comments — comment cleanup happens in plan 04-03.
  ...
  <verify>
    uv run pytest — all Python tests pass
    npm test — all Node.js tests pass
    python -c "from lib.rag_processing import _detect_footnotes_in_page, _is_ocr_corrupted, infer_written_page_numbers" — facade re-exports work
    python -c "from lib.rag_processing import _extract_and_format_toc, _extract_publisher_from_front_matter" — facade re-exports work
    python -c "from lib.rag.detection.footnotes import _detect_footnotes_in_page" — cross-module imports resolve (footnotes→utils.cache)
    python -c "from lib.rag.detection.toc import _extract_and_format_toc" — cross-module imports resolve
  </verify>
  <done>
