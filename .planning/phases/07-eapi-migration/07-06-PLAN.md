...
1. After getting the response from `client.search("test", limit=1)`, check if the response contains Cloudflare indicators. The EAPI client uses httpx internally, so Cloudflare may manifest as:
...
2. In the exception handler, classify errors:
   - If exception message contains "Checking your browser", "cloudflare", "cf-", or "challenge": return `error_code: "cloudflare_blocked"`
   - If exception is a connection/timeout error (httpx.ConnectError, httpx.TimeoutException, ConnectionError, TimeoutError): return `error_code: "network_error"`
...
4. Add a helper function `_classify_health_error(error: Exception) -> str` that returns the error code string. Keep it simple — pattern match on `str(error).lower()`.
...
Run: `cd /home/rookslog/workspace/projects/zlibrary-mcp && grep 'network_error' lib/python_bridge.py` — should match
  ...
  <done>Health check function classifies errors into cloudflare_blocked, network_error, malformed_response, or unknown_error codes.</done>
</task>
...
1. `test_health_check_detects_cloudflare`: Mock the EAPI client to raise an exception containing "Checking your browser" text. Assert response has `error_code: "cloudflare_blocked"` and `status: "unhealthy"`.

2. `test_health_check_detects_network_error`: Mock the EAPI client to raise `ConnectionError`. Assert response has `error_code: "network_error"`.
...
Use `unittest.mock.patch` or `pytest-mock` to mock `get_eapi_client` to return a mock client whose `.search()` raises the appropriate exceptions.
  </action>
  <verify>
Run: `cd /home/rookslog/workspace/projects/zlibrary-mcp && uv run pytest __tests__/python/test_python_bridge.py -k "health" -v --tb=short`
  ...
  <done>Three new health check tests pass: Cloudflare detection, network error detection, malformed response detection.</done>
</task>
...
<success_criteria>
Health check returns specific error_code field for all failure modes. Cloudflare challenge patterns are detected. Three new unit tests validate the detection logic.
</success_criteria>
