...
class TestPerformance:
    ...
    def test_detection_single_region_fast(self, benchmark):
        ...
        text = (
            "This is a sample sentence of normal English text. " * 40
        )  # ~2000 chars (typical region)
    ...
    def test_detection_long_text_scales_linearly(self):
        ...
        assert scaling_factor < 15, (
            f"Scaling factor {scaling_factor:.2f} suggests non-linear complexity"
        )
    ...
    def test_page_detection_under_100ms(self):
        ...
        assert duration < 0.1, (
            f"Page detection took {duration * 1000:.0f}ms (target: <100ms)"
        )
...
class TestScalability:
    ...
    def test_empty_string_instant(self):
        ...
        assert avg_time < 0.00001, (
            f"Empty string took {avg_time * 1000000:.2f}µs (should be <10µs)"
        )
    ...
    def test_short_text_instant(self):
        ...
        avg_time = duration / 10000
        assert avg_time < 0.00001, (
            f"Short text took {avg_time * 1000000:.2f}µs (should be <10µs)"
        )

    def test_typical_region_fast(self):
        """Typical region (~2000 chars) should be <1ms."""
        text = (
            "This is typical book text with normal sentence structure. " * 35
        )  # ~2000 chars
    ...
    def test_large_region_acceptable(self):
        ...
        text = (
            "This is a large region with substantial text content. " * 200
        )  # ~10,000 chars
        ...
        assert avg_time < 0.01, (
            f"Large region took {avg_time * 1000:.2f}ms (target: <10ms)"
        )
...
class TestMemoryEfficiency:
    ...
    def test_result_object_small(self):
        ...
        assert total_size < 1024, (
            f"Result object is {total_size} bytes (should be <1KB)"
        )
    ...
    def test_no_memory_leaks_repeated_calls(self):
        ...
        assert growth_ratio < 1.1, (
            f"Memory grew by {(growth_ratio - 1) * 100:.1f}% (should be <10%)"
        )
