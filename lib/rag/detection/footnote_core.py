"""
Main footnote detection loop and font analysis for PDF documents.

Contains the core detection function _detect_footnotes_in_page which scans
body text for footnote markers, finds definitions, applies corruption recovery,
note classification, and incomplete detection. Also contains font size calculation
and superscript detection helpers.
"""
import logging
import re
...
def _detect_footnotes_in_page(page: Any, page_num: int) -> Dict[str, List[Dict[str, Any]]]:
    ...
    # - Symbols: *, dagger, double-dagger, section, pilcrow, #
    ...
    for block in blocks:
        ...
        for line_idx, line in enumerate(block.get("lines", [])):
            ...
            for span_idx, span in enumerate(line["spans"]):
                ...
                # 3. Symbols within brackets [p. 23dagger] - HIGH CONFIDENCE
                ...
                # - Known footnote symbol (*, dagger, etc.) AND NOT at definition start OR
                ...
                # Example: After detecting '*', a standalone 't' is likely corrupted 'dagger'
                ...
                if result['markers'] and len(marker_text) <= 3 and not is_at_definition_start:
                    ...
                    corruption_chars = {
                        't',    # dagger -> 't' (85% probability)
                        'iii',  # * -> 'iii' OR double-dagger -> 'iii'
                        'tt',   # double-dagger -> 'tt'
                        's',    # section -> 's'
                        'p',    # pilcrow -> 'p'
                        'o',    # degree -> 'o'
    ...
    # Example: Derrida has both * and dagger in single block "iii ...text... t ...text..."
