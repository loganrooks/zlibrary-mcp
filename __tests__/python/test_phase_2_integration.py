...
def sample_pdf_block():
    ...
    test_pdf = (
        Path(__file__).parent.parent.parent / "test_files" / "heidegger_pages_79-88.pdf"
    )
...
class TestAnalyzePdfBlockRefactoring:
    ...
    def test_structured_path_returns_pageregion(self, sample_pdf_block):
        ...
        assert type(result).__name__ == "PageRegion", (
            "Structured path should return PageRegion"
        )
        ...
        assert hasattr(result, "heading_level"), (
            "PageRegion should have heading_level attribute"
        )
        assert hasattr(result, "list_info"), (
            "PageRegion should have list_info attribute"
        )
    ...
    def test_structured_path_creates_textspans(self, sample_pdf_block):
        ...
        assert type(first_span).__name__ == "TextSpan", (
            "Spans should be TextSpan objects"
        )
        assert hasattr(first_span, "text"), "TextSpan should have text attribute"
        assert hasattr(first_span, "formatting"), (
            "TextSpan should have formatting attribute"
        )
    ...
    def test_env_var_true_uses_structured(self, sample_pdf_block):
        ...
        try:
            result = _analyze_pdf_block(sample_pdf_block)
            assert type(result).__name__ == "PageRegion", (
                "Env var 'true' should use structured path"
            )
        finally:
            # Restore env var
            if old_val:
    ...
    def test_formatting_extraction(self, sample_pdf_block):
        ...
        _has_formatting = any(len(span.formatting) > 0 for span in result.spans)
        # Note: This may not be true for all PDFs, so we just check that the field exists
        for span in result.spans:
            assert isinstance(span.formatting, set), (
                "Each span should have formatting as set"
            )
    ...
    def test_bbox_extraction(self, sample_pdf_block):
        ...
        assert any(coord != 0.0 for coord in result.bbox), (
            "bbox should contain non-zero values"
        )
    ...
    def test_heading_level_preservation(self, sample_pdf_block):
        ...
        result = _analyze_pdf_block(
            sample_pdf_block, detect_headings=True, return_structured=True
        )
    ...
    def test_backward_compatibility_maintained(self, sample_pdf_block):
        ...
        result = _analyze_pdf_block(
            sample_pdf_block, preserve_linebreaks=False, detect_headings=True
        )
