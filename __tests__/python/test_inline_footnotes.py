...
class TestMarkerDrivenDetection:
    ...
    def test_inline_footnote_mid_page(self):
        ...
        assert len(asterisk_markers) > 0, \
            f"Asterisk marker not found (inline mid-page detection failed)"
        ...
        assert long_footnote is not None, \
            f"Inline mid-page footnote not detected.\n" \
            f"This indicates marker-driven detection failed for footnotes above bottom 20%."

        # Verify it's reasonably long (not just a fragment)
        assert len(long_footnote.get('content', '')) > 50, \
            f"Footnote too short - may have detected wrong content"

        print(f"\n✅ Inline mid-page footnote detected successfully!")
        print(f"   Location: ~58% down page (above old 75% threshold)")
    ...
    def test_traditional_bottom_footnote(self):
        ...
        assert len(definitions) >= 2, \
            f"Traditional bottom footnote definitions not detected (regression)"
    ...
    def test_mixed_inline_and_traditional(self):
        ...
        assert len(definitions) > 0, \
            "No footnotes detected on page with mixed layout"
    ...
    def test_marker_confidence_scoring(self):
        ...
        assert len(markers_with_confidence) > 0, \
            "No markers have confidence scores"
    ...
    def test_inline_spatial_threshold_50_percent(self):
        ...
        assert len(mid_page_definitions) > 0, \
            f"No definitions found in 50-80% range (inline detection failed).\n" \
            f"This indicates spatial threshold is still too restrictive."
    ...
    def test_footnote_anywhere_on_page(self):
        ...
        print(f"   Architecture supports footnotes anywhere on page")
...
class TestMarkerlessContinuation:
    ...
    def test_markerless_continuation_detected(self):
        ...
        assert markerless_found, \
            "Markerless continuation not detected on page 2"

        print(f"\n✅ Markerless continuation detected:")
        print(f"   Page 1: Incomplete footnote ending in 'to'")
        print(f"   Page 2: Continuation starting with 'which'")
    ...
    def test_markerless_continuation_confidence_scoring(self):
        ...
        print(f"   Signals: lowercase start + font match")
    ...
    def test_false_positive_markerless_content(self):
        ...
        assert len(completed) == 2, \
            f"Expected 2 completed footnotes (false incomplete handling)"
    ...
    def test_hyphenation_across_page_break(self):
        ...
        page2_notes = [{
            'marker': None,
            'content': 'standing of pure reason.',
            'is_complete': True
        }]
        completed = parser.process_page(page2_notes, page_num=2)
    ...
    def test_lowercase_start_continuation_signal(self):
        ...
        page1_notes = [{
            'marker': '1',
            'content': 'This argument refers to',
            'is_complete': False
        }]
        parser.process_page(page1_notes, page_num=1)
        ...
        assert len(completed) == 1, \
            "Lowercase start continuation not detected"
    ...
    def test_conjunction_start_continuation_signal(self):
        ...
        assert len(completed) == 1, \
            "Conjunction start continuation not detected"
...
class TestRealWorldInlineFootnotes:
    ...
    def test_kant_asterisk_inline_detected(self):
        ...
        print(f"   Location: ~58% down page (inline)")
    ...
    def test_kant_asterisk_continuation_merged(self):
        ...
        completed_page2 = parser.process_page(page2_notes, page_num=2)
        ...
        completed_page3 = parser.process_page(page3_notes, page_num=3)
    ...
    def test_kant_numeric_footnotes_still_work(self):
        ...
        assert len(numeric_markers) >= 1, \
            "Numeric footnotes not detected (regression)"
    ...
    def test_derrida_symbolic_markers_unaffected(self):
        ...
        assert len(definitions) >= 2, \
            "Symbolic markers not detected (regression)"
    ...
    def test_kant_64_65_inline_asterisk(self):
        ...
        assert len(asterisk_markers) > 0, \
            "Asterisk marker not found on page 1"

        # Verify footnote definition
        assert len(definitions_page1) > 0, \
            "No footnote definitions on page 1"
        ...
        assert incomplete_found, \
            "Incomplete footnote not detected on page 1"

        doc.close()

        print(f"\n✅ Kant 64-65 inline asterisk validated:")
        print(f"   Page 1: Asterisk marker found")
        print(f"   Page 1: Incomplete footnote ending in 'to'")
    ...
    def test_kant_64_65_continuation_pattern(self):
        ...
        assert incomplete_content is not None, \
            "Footnote ending in 'to' not found"
...
class TestPerformanceAndEdgeCases:
    ...
    def test_malformed_pdf_resilience(self):
        ...
        pdf_paths = [
            Path(__file__).parent.parent.parent / "test_files/kant_critique_pages_80_85.pdf",
            Path(__file__).parent.parent.parent / "test_files/derrida_footnote_pages_120_125.pdf"
        ]
