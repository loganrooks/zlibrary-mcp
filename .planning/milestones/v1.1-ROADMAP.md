...
Stabilize infrastructure (Node 22, EAPI gaps, Docker), improve extraction quality for scholarly texts (margin content, adaptive resolution, body text purity), and explore Anna's Archive as alternative source.
...
**Execution Order:** 8 → 9 → 10 → 11 → 12 (sequential for focus, though 10 and 12 were technically independent)
...
- RES-ADAPTIVE-DEFAULT: Adaptive DPI is default behavior; scanned PDFs auto-fallback to 300
...
- ANNAS-DOMAIN-INDEX-1: Use domain_index=1 for Anna's Archive fast download API
- SOURCE-ANNAS-PRIMARY: Anna's Archive is primary source (user has API key)
- ROUTER-QUOTA-FALLBACK: Zero downloads_left triggers fallback to LibGen

**Issues Resolved:**
- Docker numpy/Alpine compilation issue — fixed with opencv-python-headless
- EAPI booklist/full-text search gaps — improved fallbacks with enriched results
- AsyncZlib technical debt — fully removed, pure EAPI downloads

**Issues Deferred:**
- search_multi_source MCP tool wiring (Python bridge ready, TypeScript pending)
- Full Zod 4 migration (Zod 3.25.x bridge works)

**Technical Debt Incurred:**
- Deprecated AsyncZlib code still present in client_manager.py (marked deprecated)
